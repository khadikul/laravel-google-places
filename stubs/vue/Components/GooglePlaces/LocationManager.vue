...
import ConnectGoogleButton from './ConnectGoogleButton.vue'
...
const props = defineProps({
  ...
  connectRoute: { type: String, required: true },
  syncRoute: { type: String, required: true },
})
...
const sync = (locationName) =>
  router.post(props.syncRoute, { location: locationName }, { preserveScroll: true })
</script>
...
<template>
  <div class="space-y-6">
    <section class="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
      ...
      <dl v-if="connection" class="mt-3 grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
        <div>
      ...
      <template v-else>
        ...
        <ConnectGoogleButton :href="connectUrl" class="mt-4" />
      </template>
    </section>
    ...
    <section class="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
      ...
      <p v-if="connected.length === 0" class="mt-2 text-sm text-gray-500 dark:text-gray-400">
        Nothing connected yet.
      </p>

      <div v-else class="mt-3 space-y-2">
        <div
          v-for="location in connected"
        ...
        >
          ...
          <button
            ...
            @click="sync(location.location_name)"
          >
            Sync now
          </button>
        </div>
